...
def build_story_prompt(narrative: str, genres: List[str], top_artists: List[str]) -> str: # Added top_artists
    ...
    # NEW: Create personalization string
    personalization_str = ""
    if top_artists:
        personalization_str = f"\nUser's top artists (use these for inspiration): {', '.join(top_artists)}"
...
{{
  "scenes": [
    {{
      ...
      "search_query": "A precise Spotify search query for this scene. Combine musical keywords, moods, and relevant genres. Example: 'slow ambient hopeful sunrise' or 'chaotic high-energy world music market'"
    }}
  ]
}}
...
- **If the user's top artists fit the mood, try to include one (e.g., 'artist:\"Artist Name\"') in the search_query.**""" # <-- Added personalization rule
...
{{
  ...
  "search_query": "A precise Spotify search query to find this vibe. Combine the user's prompt keywords, mood, and preferred genres. Example: 'chill rainy afternoon acoustic jazz' or 'dark futuristic electronic artist:\"Gesaffelstein\"'"
}}
...
- **If the user's top artists fit the vibe, try to include one (e.g., 'artist:\"Artist Name\"') in the search_query.**""" # <-- Added personalization rule